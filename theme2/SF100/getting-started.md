...
small so LNX201 will be the perfect home.
...
the cluster are large and powerful. LNX201 has 16 CPUs ("cores", sort
...
screaming, and too hot to handle.
...
computer in a far-flung destination and shows what is going on right
in front of you. The effect is to make your computer's screen,
keyboard, and mouse act like they're connected to the remote machine,
albeit with a really, really long set of cables. The second approach
is to use a program on your laptop or desktop to connect over the
network to a remote machine and just pass characters back and
forth. It gives you, for all practical purposes, something that looks
like a window for using the command line, only the computer running
those commands is remote.
...
1 - The DUO app installed on your phone and configured for access to CLASSE
1 - The "Pritunl" Virtual Private Network software installed on your laptop.
...
Go ahead and connect to the CLASSE VPN (internal network) through the Pritunl software. Then go to your web browser
and put in the destination URL [https://nomachine.classe.cornell.edu][nomachine-classe]. In just a moment, you should
be seeing the initial login screen for NoMachine itself:


Put in your username and password, then click on "OK" in the lower right corner.
...
available session types. In principle, you could create different
...
actual desktop GUI, all the sessions use simulated (virtual) desktops
and so they can be set to any resolution you want. Selecting "Don't
resize" is a reasonable starting point. You might want to experiment
with the others later. In any case, we can finally click on "OK" for
this screen and connect to the desktop session!
...
Nonetheless, the most common thing we'll do on a remote desktop
session is "start a terminal window". In this case, it's dead simple:
...
more or equivalent to Explorer or Finder. Double Click on the "Home"
folder...
...
At this point, we've how to log in to the Linux cluster through a
...
goodness.  All good things must come to an end, eventually, and this
...
home refridgerator and the server didn't even have a keyboard. To
...
late 1970s, teletypes were getting rare because they had been replaced
by "terminals" (a keyboard, a picture tube, and enough electronics to
draw the characters on the screen). The connection to the computer was
still a serial cable. To this day, you can plug a USB-to-Serial
converter into a Linux machine, set up a configuration file, and log
in this way. It's fun a time or two, but the novelty wears off.
...
at any speed much over 1200 bits per second. Covering a whole campus
required a network of some sort, and what the research and academic
world standardized on was "Internet Protocol" (IP). One of the first
programs ever written for IP was a program called "telnet". This
program used the network to set up a stream of characters to and from
a remote machine to the local one. It was like having a serial cable,
except it was literally a thousand times faster and could reach around
the world. To this day, telnet is still the way to use Internet Relay
Chat (IRC).

Telnet was great in the early days of networking. But we were so naive
back then. Anyone who could get access to the network could listen in
whatever traffic was going over these telnet connections. Most of the
traffic was utterly boring - email, experimental results, and some
command lines. But there was one thing worth snooping on, and it was
the first two things a user typed when they set up a connection: their
username and their password.  In the late 1980s and early 1990s, this
was mostly a theoretical concern. The problem was the explosive growth
of the Internet in the "Dot Com" era (1995-2000). Once there were a
critical mass of Internet users, and once these users started using
the Internet for things that were _worth actual money_, security
started moving front and center. By this point, sending anything over
the internet without encrypting it first became a VBA (Very Bad
Idea). Thus was born the Secure SHell (SSH).
...
Either way, you're now looking at a terminal window of some sort. Characters are entered and sent to some program, and programs send characters back out and the terminal window draws them on the screen. Just like a teletype, except no paper and no oily smell. Windows users will probably see something similar to ```C:\>```.  Mac users will see something more akin to ```escott@RENCI_LP91DX62MX ~ %```.  Both of these are "prompts", meaning "you can type a command here". In both cases the command to type is ```ssh lnx201.classe.cornell.edu```. The lower case letter in "ssh" are important. Linux commands _almost_ never contain uppercase letters. Go ahead, enter that ssh command, and press Enter.  If this is the very first time you've ssh'ed into lnx201 from the machine you're at, you'll get some warnings just to remind you to be wary:
...
refers to a encrypted, secure way to tell we're reaqlly using the
...
against this, but it's a (minor?) hassle so it's rarely done. But
...
and not continue until we're comfortable we know why.
...
SSH has a few more tricks up its sleeve. One is that it can copy files. When you copy files this way you use the "scp' command instead of "ssh", but it's really the same program behind the scenes. Another trick is that it can do "port forwarding" - a poor substitute for a VPN, but sometimes you're in a situation where you're not allowed to install a VPN to get to your instrument, for instance. And finally, yes, I have to acknowledge this for the nerds who've been waiting to ambush me... ssh _can_ be used to run graphical programs remotely. The "-X" and "-Y" options are the relevant ones. Results are "good enough" for simple programs over very fast networks, but slower networks make the experience miserable and complicated graphical programs (Chrome? Just saying.) are awful no matter how fast the network is.
